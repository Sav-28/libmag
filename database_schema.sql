...
BEGIN
    ...
    IF NEW.available_copies = 0 AND OLD.available_copies > 0 THEN
        ...
        INSERT INTO book_notifications (book_id, title, author, notification_type, notification_date, message)
        VALUES (NEW.book_id, NEW.title, NEW.author, 'empty', NOW(), 
                CONCAT('Book "', NEW.title, '" by ', NEW.author, ' is now out of stock'));
...
BEGIN
    ...
    IF NEW.available_copies > 0 AND OLD.available_copies = 0 THEN
        INSERT INTO book_notifications (book_id, title, author, notification_type, notification_date, message)
        VALUES (NEW.book_id, NEW.title, NEW.author, 'restocked', NOW(), 
                CONCAT('Book "', NEW.title, '" by ', NEW.author, ' has been restocked (', NEW.available_copies, ' copies available)'));
    END IF;
END$$
...
BEGIN
    ...
    IF NEW.available_copies <= 2 AND OLD.available_copies > 2 AND NEW.available_copies > 0 THEN
        INSERT INTO book_notifications (book_id, title, author, notification_type, notification_date, message)
        VALUES (NEW.book_id, NEW.title, NEW.author, 'low_stock', NOW(), 
                CONCAT('Book "', NEW.title, '" by ', NEW.author, ' is running low (', NEW.available_copies, ' copies remaining)'));
    END IF;
END$$
